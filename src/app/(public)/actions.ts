...
async function generateOrReusePix(payment: PaymentRow, description: string): Promise<PayState> {
  if (payment.status === "paid") {
    return { status: "already-paid" };
  }

  if (payment.pix_copy_paste) {
    return { status: "pix-ready", copyPaste: payment.pix_copy_paste, amount: payment.amount };
  }
  ...
  try {
    ...
    await attachPixToPaymentPublic(payment.id, {
      mercadoPagoPaymentId: pix.mercadoPagoPaymentId,
      qrCode: pix.qrCode,
      qrCodeBase64: pix.qrCodeBase64,
    });

    return { status: "pix-ready", copyPaste: pix.qrCode, amount: payment.amount };
  } catch (err) {
...
export async function payMensalidadeAction(playerId: string): Promise<PayState> {
  try {
    ...
    return await generateOrReusePix(payment, "Mensalidade do time");
  } catch {
...
export async function payCaixinhaAction(
  ...
  try {
    ...
    return await generateOrReusePix(payment, "Caixinha do time");
  } catch {
    return { status: "error", message: "Não foi possível iniciar o pagamento." };
  }
}
